 ...
 * @version 1.0
 */
public class Feed extends com.corelib.CoreLib {
	/**
	 * The class tag name
	 */
	...
	public void processFeed() throws Exception {
		...
		try {
			...
			// Validate feed
			if(data.equals("")) {
				throw new Exception(this.TAG + ":" + this.ERROR_EMPTY_FEED);
			}
			...
			if(doc == null) {
				throw new Exception(this.TAG + ":" + this.ERROR_INVALID_DOM);
			}

			// Loop through the nodes
			String parent = this.KEY_RSS_PARENT;
			if(this.mode == Feed.SITEMAP) {
				parent = this.KEY_SITEMAP_PARENT;
			}
